...
This page explains why the lab uses **four** tokenizers instead of only comparing one custom tokenizer against one general tokenizer.

The short answer is fairness. If two tokenizers differ in too many ways at once, you cannot tell which difference caused the result.

## The 2×2 design

The diagram below is easiest to read as a grid:

- **small vs large vocabulary**
- **general vs medical training domain**

That structure lets the lab separate two different questions:

1. does domain matter?
2. does vocabulary size matter?
...
The most important pair in this diagram is `general-bpe` versus `custom-med`.

Why? Because they hold almost everything constant:

- same byte-level BPE algorithm
- same 16k vocabulary size
- same evaluation method

The main thing that changes is the training corpus. That is what makes this a real control.

## What each tokenizer proves

Each tokenizer has a different job in the argument:

- `cl100k` shows what a strong general tokenizer does
- `o200k` shows what an even larger general tokenizer does
- `general-bpe` shows what happens when vocab size is matched but domain is still general
- `custom-med` shows what happens when both the corpus and the tokenizer are specialized for medicine
...
The held-out evaluation sets matter just as much as the tokenizers. They make the comparison honest:

- medical held-out text checks in-domain performance
- general held-out text checks whether the tokenizer truly specialized
...
This is the core logic of the lab.

If `custom-med` only beat `cl100k`, someone could argue that the result came from some other difference in tokenizer design or training setup.

But once `custom-med` also beats `general-bpe`, that argument gets much weaker, because the comparison is now much cleaner.

## The isolation argument

This final diagram shows the reasoning step by step.

It starts with a question: why did `custom-med` win on medical text?

Then it tests the main competing explanations one by one.
...
In plain language: the lab is not just showing that `custom-med` wins. It is showing **why** it wins.

That is what makes this a good experiment instead of just a demo.
